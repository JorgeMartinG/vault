...
// API functions.
async function deleteUploadedFile(filename) { // -> delete file function.
    try {
        const response = await fetch(
            CONFIG.API_URL + CONFIG.ENDPOINTS.FILES + filename, {
                method: 'DELETE',
            });
        if (response.ok) {
...
function testMessage() {
    showMessage("This is an example message");
}
async function processVideo(filename) {
    try {
        const response = await fetch(CONFIG.API_URL + CONFIG.ENDPOINTS.PROCESS + filename, { 
                method: 'POST',
...
async function pollProcessingStatus(taskId) {
   const pollInterval = setInterval(async () => {
       try {
            const response = await fetch(CONFIG.API_URL + CONFIG.ENDPOINTS.STATUS + taskId, {
                method: 'GET',
            });
            if (response.ok) {
               const data = await response.json();
...
async function fetchProcessedFiles() {
   try {
       const response = await fetch('http://192.168.1.146:8000/api/processed/');
       const data = await response.json();
...
async function fetchUploadedFiles() {
   try {
       const response = await fetch('http://192.168.1.146:8000/api/files/');
       const data = await response.json();
